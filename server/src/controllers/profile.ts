...
export const getAutoGeneratedPlaylist: RequestHandler = async (req, res) => {
  ...
  const agpls = await AutoGeneratedPlaylist.aggregate([
    matchOptions,
    { $sample: { size: 4 } },
    {
      $project: {
...
export const getPlaylistAudios: RequestHandler = async (req, res) => {
  ...
  if (!playlistResult) {
    const [autoPlaylitResult] = await AutoGeneratedPlaylist.aggregate(
      aggregationLogic
    );
    return res.json({ data: autoPlaylitResult });
  }
...
export const getIsFollowing: RequestHandler = async (req, res) => {
  ...
  if (!isValidObjectId(profileId))
    return res.status(422).json({ error: "Invalid profiel id" });
